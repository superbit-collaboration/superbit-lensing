...
def remove_low_redshift(tab, ned, redshift_cut, tolerance_deg):
    ...
    if len(remove_idx) == 0:
        print(f"  NED low-z cut (z < {redshift_cut:.4f}): "
              f"0/{len(tab)} removed, {len(tab)} remaining")
        return tab
...
def run(cfg):
    ...
    if cfg['foreground_file'] is not None and os.path.exists(cfg['foreground_file']):
        print(f"The Foreground file is: {cfg['foreground_file']}")
    ...
    elif not os.path.exists(cfg['foreground_file']):
        print(f"Foreground file ({cfg['foreground_file']}) not found — skipping known foreground cut")
    else:
        print("No foreground file provided — skipping known foreground cut")
    
    if cfg['boundary_cuts'] is not None:
        boundary_cuts = cfg['boundary_cuts']
        ra_max = boundary_cuts['max_ra']
        ra_min = boundary_cuts['min_ra']
        dec_max = boundary_cuts['max_dec']
        dec_min = boundary_cuts['min_dec']

        print(f"  Boundary cuts applied:")
        print(f"    RA:   {ra_min:.4f} to {ra_max:.4f}")
        print(f"    Dec:  {dec_min:.4f} to {dec_max:.4f}")
        print(f"  Applying boundary cuts...")
        tab = tab[
            (tab['ra'] >= ra_min) & (tab['ra'] <= ra_max) &
            (tab['dec'] >= dec_min) & (tab['dec'] <= dec_max)
        ]
