...
def mp_fit_one(obslist, prior, rng, psf_model='gauss', gal_model='gauss', mcal_pars= DEFAULT_MCAL_PARS):
    ...
    guess_gm = GaussMom(1.2).go(obslist[0])
    Tguess = guess_gm['T']
    flux_guess = guess_gm['flux']
    ntry = 20
    ...
    if psf_model =='galsim':
        ...
        fitter = ngmix.fitting.GalsimFitter(model=gal_model, prior=prior, fit_pars=lm_pars)
        psf_runner = None
...
def mp_run_fit(i, obj, obslist, prior,
               logprint, rng, psf_model='gauss', gal_model='gauss', mcal_pars= DEFAULT_MCAL_PARS):
    '''
